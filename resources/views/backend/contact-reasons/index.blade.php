...
@section('breadcrumb')
<li class="breadcrumb-item active" aria-current="page">{{ CONTACT_REASON_LABEL }} List</li>
@endsection
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <div class="col-sm-6">
                        <h3 class="card-title">{{ CONTACT_REASON_LABEL }}</h3>
                    </div>
                    <div class="col-sm-6 text-right" style="float:right">
                        <a class="btn btn-primary p-1" href="{{ route('backend.contact-reasons.create') }}"><i
                                class="fas fa-plus" aria-hidden="true"></i> Add Contact Reason</a>
                    </div>
                </div><!-- /.row -->
                <!-- /.card-header -->
                <div class="card-body">
                    <table id="js-contact-reasons-table"
                            class="table table-bordered table-striped table-hover faq-table">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Contact Reasons</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body p-4 py-5 text-center">
                ...
                <h4 class="bold mt-3">Delete Contact Reason</h4>
...
<script>
    function initializeTable(elementSelector) {
        ...
        $(elementSelector).on('preXhr.dt', function (e, settings, data) {
            ...
            ajax: {
                url: "{{ route('backend.contact-reasons.index') }}",
                beforeSend: function () {
                    $('#js-contact-reasons-table tbody').html(
            ...
            initComplete: function () {
                var r = $('#js-contact-reasons-table tfoot tr');
                $('#js-contact-reasons-table thead').append(r);
            ...
            drawCallback: function () {
                ...
                $('#js-contact-reasons-table_paginate span').addClass('page-item').css('display', 'flex')
                $('#js-contact-reasons-table_paginate span > a').addClass('page-link')
            }
        });
    }


    $(document).on('click', '.delete-contact-reason', function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        initializeTable('#js-contact-reasons-table');
    });
</script>
@endpush
